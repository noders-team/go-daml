...
func TestGetMainDalf(t *testing.T) {
	...
	res, err := Bind("main", ast.Name, ast.Version, manifest.SdkVersion, ast.Structs, true)
	require.NoError(t, err)
...
func TestGetMainDalfV3(t *testing.T) {
	...
	res, err := Bind("codegen_test", ast.Name, ast.Version, manifest.SdkVersion, ast.Structs, true)
	require.NoError(t, err)
	require.NotEmpty(t, res)
