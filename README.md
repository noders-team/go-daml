...
- **Typed Contract Queries** - Generic `NewContractQuery[T]` helper that fetches active contracts by template and decodes them directly into code-generated structs
...
### Example 5: Typed Contract Queries with `NewContractQuery`

`client.NewContractQuery[T]` is a generic helper for fetching active contracts and
decoding them straight into a struct generated by the `godaml` code generator. It scans
the active contract set filtered by template and returns `[]client.Contract[T]`, where
each entry pairs the contract ID with the decoded `Data` — no manual `RecordToStruct`
calls or event-loop boilerplate.

Pass the template ID from the generated contract's `GetTemplateID()` method, which emits
the `#package-name:Module:Entity` reference resolved by the ledger.
...
func main() {
    ...
    // MappyContract is a struct emitted by the godaml code generator.
    var tmpl MappyContract
    query := client.NewContractQuery[MappyContract](cl)

    contracts, err := query.FindContractsByTemplate(ctx, "Alice", tmpl.GetTemplateID())
    if err != nil {
        log.Fatal().Err(err).Msg("failed to query contracts")
    }

    for _, c := range contracts {
        log.Info().
            Str("contractID", c.ContractID).
            Str("operator", string(c.Data.Operator)).
            Msg("found typed contract")
    }
}
```

`FindContractsByTemplate(ctx, partyID, templateID)` returns every active contract of the
given template visible to `partyID`, each decoded into `T`:
