...
const (
	...
	BuiltinFunction_SECP256K1_VALIDATE_KEY         BuiltinFunction = 73   // *Available in versions >= 2.3
...
type Update_ExerciseByKey_ struct {
	ExerciseByKey *Update_ExerciseByKey `protobuf:"bytes,10,opt,name=exercise_by_key,json=exerciseByKey,proto3,oneof"` // from 2.3
}
...
type Update_LookupByKey struct {
	LookupByKey *Update_RetrieveByKey `protobuf:"bytes,8,opt,name=lookup_by_key,json=lookupByKey,proto3,oneof"` // from 2.dev
}

type Update_QueryNByKey_ struct {
	QueryNByKey *Update_QueryNByKey `protobuf:"bytes,16,opt,name=query_n_by_key,json=queryNByKey,proto3,oneof"` // from 2.3
}

type Update_FetchByKey struct {
	FetchByKey *Update_RetrieveByKey `protobuf:"bytes,9,opt,name=fetch_by_key,json=fetchByKey,proto3,oneof"` // from 2.3
}
...
// used as both FetchByKey and LookupByKey (both queries have same parameters, but different type/auth rules)
...
const file_com_digitalasset_daml_lf_archive_daml_lf2_proto_rawDesc = "" +
	...
	"\x13TYPE_REP_TYCON_NAME\x10\xc3\x17B\x9e\x01\n" +
	" com.digitalasset.daml.lf.archiveZOgithub.com/noders-team/go-daml/proto/com/digitalasset/daml/lf/archive/daml_lf_2\xaa\x02(Com.DigitalAsset.Daml.Lf.Archive.DamlLf2b\x06proto3"
